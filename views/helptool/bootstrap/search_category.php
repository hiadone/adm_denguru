<div class="modal-header">
    <h4 class="modal-title">게시물 카테고리</h4>
</div>
            ...
            <?php
                ...
                if (element(0, $category)) {
                    ...
                    foreach (element(0, $category) as $key => $val) {
                        ...
                        echo '<label for="cca_id_' . element('cca_id', $val) . '"><input type="checkbox" name="cmall_category[]" value="' . element('cca_id', $val) . '" ' . $cat_checked . ' id="cca_id_' . element('cca_id', $val) . '" onclick="display_cmall_category(this.checked,\'catwrap_' . element('cca_id', $val) . '\');" label="' . element('cca_value', $val) . '" />' . element('cca_value', $val) . '</label> ';
                        echo get_subcat($category, $item_category, element('cca_id', $val), 'block');
                        echo '</div>';
                        if ($i%3== 2) {
                            echo '</div>';
                            $open = false;
                ...
                function get_subcat($category, $item_category, $key, $display)
                {
                    ...
                    if ($subcat) {
                        ...
                        foreach ($subcat as $skey => $sval) {
                            ...
                            $html .= '<label for="cca_id_' . element('cca_id', $sval) . '"><input type="checkbox" name="cmall_category[]" value="' . element('cca_id', $sval) . '" ' . $cat_checked . ' id="cca_id_' . element('cca_id', $sval) . '" onclick="display_cmall_category(this.checked,\'catwrap_' . element('cca_id', $sval) . '\');" label="' . element('cca_value', $sval) . '" /> ' . element('cca_value', $sval) . '</label>';
...
function add() {
    ...
    $("input[name='cmall_category[]']:checked").each(function (index) {  
