...
class Boardgroup extends CB_Controller
{
	 ...
	 * 모델을 로딩합니다
	 */
	protected $models = array('Board_group', 'Board_group_meta', 'Board', 'Board_group_admin','Board_group_category');
	...
	public function write_category($pid = 0)
	{
		...
		if ($this->form_validation->run() === false) {
			...
			if ($this->input->post('type') === 'add') {
				...
				$bca_key = $this->Board_group_category_model->next_key($bca_parent, $pid);
				...
				$this->Board_group_category_model->insert($insertdata);
				$this->cache->delete('category-' . $pid);
				$this->cache->delete('category-all-' . $pid);

				$view['view']['alert_message'] = '카테고리 설정이 저장되었습니다';
				redirect(admin_url($this->pagedir.'/write_category/' . $pid), 'refresh');
			}
			if ($this->input->post('type') === 'modify') {
				...
				$this->Board_group_category_model->update($this->input->post('bca_id'), $updatedata);
				$this->cache->delete('category-' . $pid);
				$this->cache->delete('category-all-' . $pid);
		...
		$getdata = $this->Board_group_category_model->get_all_category($pid);
		$view['view']['data'] = $getdata;
		$view['view']['data']['bgr_id'] = $pid;
	...
	public function write_category_delete($pid = 0, $bca_id = 0)
	{
		...
		$this->Board_group_category_model->delete($bca_id);
		$this->cache->delete('category-' . $pid);
		$this->cache->delete('category-all-' . $pid);
		...
		$param =& $this->querystring;
		$redirecturl = admin_url($this->pagedir.'/write_category/' . $pid);
		redirect($redirecturl);
	}
