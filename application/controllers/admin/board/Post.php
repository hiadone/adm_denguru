...
class Post extends CB_Controller
{
	...
	public function index()
	{
		...
		if($this->input->get('noitem')){
			...
			$this->{$this->modelname}->set_where("brd_id not in (320,321) and  post_id not in(select post_id from cb_cmall_item group by post_id)",'',false);
		}

		if($this->input->get('trash'))
			$this->{$this->modelname}->set_where("post_id in(select post_id from cb_cmall_item where cit_is_del = 1 group by post_id) and brd_id not in (320,321) and post_id not in(select post_id from cb_cmall_item where cit_is_del = 0 group by post_id)",'',false);
