...
class Boards extends CB_Controller
{
	 ...
	 * 게시판관리> 글쓰기>메타사이트정보
	 */
	public function write_crawl($pid = 0)
	{
		// 이벤트 라이브러리를 로딩합니다
		$eventname = 'event_admin_board_boards_write_crawl';
		$this->load->event($eventname);
		...
		$primary_key = $this->Board_crawl_model->primary_key;
		...
		$crawlwhere = array('brd_id' => $pid);
		$getdata = $this->Board_crawl_model->get_one('', '', $crawlwhere);
		if (empty($getdata)) {
			$getdata['brd_id'] = $pid;
		} 
		...
		if ($this->form_validation->run() === false) {
			...
			if (element('bdc_id',$getdata)) {
				...
				$bdc_id = $this->Board_crawl_model->insert($updatedata);
		 ...
		 * 어드민 레이아웃을 정의합니다
		 */
		$layoutconfig = array('layout' => 'layout', 'skin' => 'write_crawl');
		$view['layout'] = $this->managelayout->admin($layoutconfig, $this->cbconfig->get_device_view_type());
		$this->data = $view;
		$this->layout = element('layout_skin_file', element('layout', $view));
		$this->view = element('view_skin_file', element('layout', $view));
	...
	public function listupdate()
	{
		...
		if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
			...
			$brd_blind = $this->input->post('brd_blind');
			...
			foreach ($this->input->post('chk') as $val) {
				if ($val) {
					...
					$brd_blind_update = element($val, $brd_blind) ? 1 : 0;
