...
class Event extends CB_Controller
{
    ...
    public function index()
    {
        ...
        $findex = $this->input->get('findex', null, 'eve_id');
    ...
    public function lists($eve_id)
    {
        ...
        $view['view']['sort'] = array(
            ...
            'evr_order' => $param->sort('(0.1/evr_order)', 'asc'),            
            'evr_start_date' => $param->sort('evr_start_date', 'asc'),
            'evr_end_date' => $param->sort('evr_end_date', 'asc'),
        );
        $findex = $this->input->get('findex', null, '(0.1/evr_order)');
        $forder = $this->input->get('forder', null, 'desc');
        $sfield = $this->input->get('sfield', null, '');
        $skeyword = $this->input->get('skeyword', null, '');
        ...
        $where = array();
        if ($eve_id) {
            $where = array('event.eve_id' => $eve_id);
        }
        ...
        // $this->{$this->modelname}->search_field_equal = array('evr_id', 'eve_id', 'kinditem.mem_id'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->{$this->modelname}->allow_order_field = array('(0.1/evr_order)', 'evr_start_date', 'evr_end_date'); // 정렬이 가능한 필드
        $result = $this->{$this->modelname}
        ...
        if (element('list', $result)) {
            foreach (element('list', $result) as $key => $val) {
                ...
                // $result['list'][$key]['kinditemgroup'] = $this->kinditem_group_model->get_one(element('eve_id', $val));
                $result['list'][$key]['num'] = $list_num--;
            }
        }
         ...
         * primary key 정보를 저장합니다
         */
        $view['view']['primary_key'] = 'evr_id';
        $view['view']['eve_id'] = $eve_id;

        /**
         * 페이지네이션을 생성합니다
         */
        $config['base_url'] = admin_url($this->pagedir.'/lists/'.$eve_id) . '?' . $param->replace('page');
        $config['total_rows'] = $result['total_rows'];
        $config['per_page'] = $per_page;
        $this->pagination->initialize($config);
        $view['view']['paging'] = $this->pagination->create_links();
        ...
        $view['view']['skeyword'] = ($sfield && array_key_exists($sfield, $search_option)) ? $skeyword : '';
        $view['view']['search_option'] = search_option($search_option, $sfield);
        $view['view']['listall_url'] = admin_url('page/event');
        $view['view']['write_url'] = admin_url($this->pagedir . '/write?eve_id=' . $eve_id);
        $view['view']['list_delete_url'] = admin_url($this->pagedir . '/itemlistdelete/'.$eve_id.'?' . $param->output());
    ...
    public function listswrite($pid = 0)
    {
        ...
        $primary_key = $this->Event_rel_model->primary_key;
        ...
        if ($pid) {
            ...
            $getdata = $this->Event_rel_model->get_one($pid);
        ...
        if ($this->form_validation->run() === false) {
            ...
            // $view['view']['data']['kinditemgroup'] = $this->Kinditem_group_model->get_one($eve_id);
            ...
            $evr_order = $this->input->post('evr_order') ? $this->input->post('evr_order') : 0;

            $evr_start_date = $this->input->post('evr_start_date');
            if ( ! $evr_start_date) $evr_start_date = null;
            $evr_end_date = $this->input->post('evr_end_date');
            if ( ! $evr_end_date) $evr_end_date = null;
            
            $updatedata = array(                
                'cit_id' => $this->input->post('cit_id', null, ''),
                'evr_order' => $this->input->post('evr_order', null, ''),
                'evr_start_date' => $evr_start_date,
                'evr_end_date' => $evr_end_date,
             ...
             */
            if ($this->input->post($primary_key)) {
                $this->Event_rel_model->update($this->input->post($primary_key), $updatedata);
                $this->session->set_flashdata(
                    'message',
                    '정상적으로 수정되었습니다'
                );
                ...
                $this->Event_rel_model->insert($updatedata);
                $this->session->set_flashdata(
                    'message',
                    '정상적으로 입력되었습니다'
                );
             ...
             */
            $param =& $this->querystring;
            $redirecturl = admin_url($this->pagedir . '/lists/'.element('eve_id',$getdata).'?' . $param->output());
            redirect($redirecturl);
        }
    }

    public function itemlistdelete($eve_id)
    {   
        ...
        if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
            foreach ($this->input->post('chk') as $val) {
                if ($val) {
                    ...
                    $this->Event_rel_model->delete_where($deletewhere);
                }
            }
        }
