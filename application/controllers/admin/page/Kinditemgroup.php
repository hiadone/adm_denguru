...
class Kinditemgroup extends CB_Controller
{
     ...
     * 모델을 로딩합니다
     */
    protected $models = array('Cmall_kind','Kinditem_rel');

    /**
     * 이 컨트롤러의 메인 모델 이름입니다
     */
    protected $modelname = 'Cmall_kind_model';
    ...
    public function write($pid = 0)
    {
        ...
        if ($form_validation) {
            ...
            if (isset($_FILES) && isset($_FILES['egr_image']) && isset($_FILES['egr_image']['name']) && $_FILES['egr_image']['name']) {
                $upload_path = config_item('uploads_dir') . '/kinditemgroup/';
        ...
        if ($form_validation === false OR $file_error !== '' OR $file_error2 !== '') {
            ...
            if ($this->input->post($primary_key)) {
                ...
                if (element('egr_image', $getdata) && ($this->input->post('egr_image_del') OR $updatephoto)) {
                    // 기존 파일 삭제
                    @unlink(config_item('uploads_dir') . '/kinditemgroup/' . element('egr_image', $getdata));

                    $deleted = $this->aws_s3->delete_file(config_item('s3_folder_name') . '/kinditemgroup/' . element('egr_image', $getdata));
                }
                ...
                if (element('egr_detail_image', $getdata) && ($this->input->post('egr_detail_image_del') OR $updatephoto2)) {
                    // 기존 파일 삭제
                    @unlink(config_item('uploads_dir') . '/kinditemgroup/' . element('egr_detail_image', $getdata));

                    $deleted = $this->aws_s3->delete_file(config_item('s3_folder_name') . '/kinditemgroup/' . element('egr_detail_image', $getdata));
                }
    ...
    public function notification_send($pid)
    {
        ...
        if (element('list', $result)) {
            foreach (element('list', $result) as $key => $val) {
                ...
                $egr_file ='';
                if(element('egr_image',$getdata))
                    $egr_file =  cdn_url('kinditemgroup', element('egr_image', $getdata));
