...
class Theme extends CB_Controller
{
     ...
     * 모델을 로딩합니다
     */
    protected $models = array('Theme','Board','Theme_rel');
    ...
    public function index()
    {
        ...
        if (element('list', $result)) {
            foreach (element('list', $result) as $key => $val) {
                ...
                $countwhere = array(
                    'the_id' => element('the_id', $val),
                );

                $result['list'][$key]['boardcount'] = $this->Theme_rel_model->count_by($countwhere);
        ...
        if ($this->input->post('chk') && is_array($this->input->post('chk'))) {
            foreach ($this->input->post('chk') as $val) {
                if ($val) {
                    ...
                    $this->Theme_rel_model->delete_where($deletewhere);
    ...
    public function lists($the_id)
    {
        ...
        $view['view']['sort'] = array(
            ...
            'thr_order' => $param->sort('(0.1/thr_order)', 'asc'),            
            'thr_start_date' => $param->sort('thr_start_date', 'asc'),
            'thr_end_date' => $param->sort('thr_end_date', 'asc'),
        );
        $findex = $this->input->get('findex', null, '(0.1/thr_order)');
        $forder = $this->input->get('forder', null, 'desc');
        $sfield = $this->input->get('sfield', null, '');
        $skeyword = $this->input->get('skeyword', null, '');
        ...
        $where = array();
        if ($the_id) {
            $where = array('theme.the_id' => $the_id);
        }

        /**
         * 게시판 목록에 필요한 정보를 가져옵니다.
         */
        $this->{$this->modelname}->allow_search_field = array('brd_name'); // 검색이 가능한 필드
        // $this->{$this->modelname}->search_field_equal = array('thr_id', 'thr_id', 'kinditem.mem_id'); // 검색중 like 가 아닌 = 검색을 하는 필드
        $this->{$this->modelname}->allow_order_field = array('(0.1/thr_order)', 'thr_start_date', 'thr_end_date'); // 정렬이 가능한 필드
        $result = $this->{$this->modelname}
        ...
        if (element('list', $result)) {
            foreach (element('list', $result) as $key => $val) {
                ...
                // $result['list'][$key]['kinditemgroup'] = $this->kinditem_group_model->get_one(element('thr_id', $val));
                $result['list'][$key]['num'] = $list_num--;
            }
        }
         ...
         * primary key 정보를 저장합니다
         */
        $view['view']['primary_key'] = 'thr_id';
        $view['view']['the_id'] = $the_id;
        ...
        $view['view']['skeyword'] = ($sfield && array_key_exists($sfield, $search_option)) ? $skeyword : '';
        $view['view']['search_option'] = search_option($search_option, $sfield);
        $view['view']['listall_url'] = admin_url('page/theme');
        $view['view']['write_url'] = admin_url($this->pagedir . '/write?the_id=' . $the_id);
        $view['view']['list_delete_url'] = admin_url($this->pagedir . '/itemlistdelete/'.$the_id.'?' . $param->output());
    ...
    public function listswrite($pid = 0)
    {
        ...
        $primary_key = $this->Theme_rel_model->primary_key;
        ...
        if ($pid) {
            ...
            $getdata = $this->Theme_rel_model->get_one($pid);
            $getdata2 = $this->Board_model->get_one(element('brd_id',$getdata));
        }
        ...
        if ($this->form_validation->run() === false) {
            ...
            // $view['view']['data']['kinditemgroup'] = $this->Kinditem_group_model->get_one($the_id);
            ...
            $thr_order = $this->input->post('thr_order') ? $this->input->post('thr_order') : 0;

            $thr_start_date = $this->input->post('thr_start_date');
            if ( ! $thr_start_date) $thr_start_date = null;
            $thr_end_date = $this->input->post('thr_end_date');
            if ( ! $thr_end_date) $thr_end_date = null;
            
            $updatedata = array(                
                'brd_id' => $this->input->post('brd_id', null, ''),
                'thr_order' => $this->input->post('thr_order', null, ''),
                'thr_start_date' => $thr_start_date,
                'thr_end_date' => $thr_end_date,
             ...
             */
            if ($this->input->post($primary_key)) {
                $this->Theme_rel_model->update($this->input->post($primary_key), $updatedata);
                $this->session->set_flashdata(
                    'message',
                    '정상적으로 수정되었습니다'
                );
                ...
                $this->Theme_rel_model->insert($updatedata);
                $this->session->set_flashdata(
                    'message',
                    '정상적으로 입력되었습니다'
                );
             ...
             */
            $param =& $this->querystring;
            $redirecturl = admin_url($this->pagedir . '/lists/'.element('the_id',$getdata).'?' . $param->output());
            redirect($redirecturl);
        }
    }

    public function itemlistdelete($the_id)
    {   
