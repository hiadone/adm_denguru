...
class Memberleave_model extends CB_Model
{
    ...
    public function get_leave_count($type = 'd', $start_date = '', $end_date = '', $orderby = 'asc')
    {
        ...
        $this->db->select('count(*) as cnt, left(mem_leave_datetime, ' . $left . ') as day ', false);
        $this->db->where('left(mem_leave_datetime, 10) >=', $start_date);
        $this->db->where('left(mem_leave_datetime, 10) <=', $end_date);
        $this->db->where('mem_denied', 0);
        $this->db->group_by('day');
        $this->db->order_by('mem_leave_datetime', $orderby);
