...
class Crawl_tag_delete_model extends CB_Model
{

    /**
     * 테이블명
     */
    public $_table = 'crawl_tag_delete';

    /**
     * 사용되는 테이블의 프라이머리키
     */
    public $primary_key = 'ctd_id'; // 사용되는 테이블의 프라이머리키
    ...
    public function get_crawl_tag_delete_count($type = 'd', $start_date = '', $end_date = '', $brd_id = 0)
    {
    ...
    {
        $select = 'crawl_tag_delete.*, crawl.mem_id as crawl_mem_id, crawl.crawl_userid, crawl.crawl_nickname, crawl.brd_id, crawl.crawl_datetime, crawl.crawl_hit, crawl.crawl_secret, crawl.crawl_title';
        $join[] = array('table' => 'crawl', 'on' => 'crawl_tag_delete.crawl_id = crawl.crawl_id', 'type' => 'inner');
     ...
     * List 페이지 커스테마이징 함수
     */
    public function get_tag_delete_list ($limit = '', $offset = '', $tag = '')
    {
        if (empty($tag)) {
            return false;
        }
        ...
        $this->db->join('crawl_tag_delete', 'crawl.crawl_id = crawl_tag_delete.crawl_id', 'inner');
